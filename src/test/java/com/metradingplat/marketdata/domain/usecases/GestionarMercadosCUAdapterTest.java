...
class GestionarMercadosCUAdapterTest {
    ...
    void obtenerSimbolosPorMercados_loadsFromGatewayOnFirstCall() {
        ...
        // Single page of results (less than PAGE_SIZE=5000)
        when(gateway.getActiveEquities(0, 5000)).thenReturn(List.of(aapl, msft, ibm));
    ...
    void obtenerSimbolosPorMercados_returnsAllWhenNoFilterSpecified() {
        ...
        when(gateway.getActiveEquities(0, 5000)).thenReturn(List.of(aapl, ibm));
    ...
    void obtenerSimbolosPorMercados_usesCacheOnSecondCall() {
        ...
        when(gateway.getActiveEquities(0, 5000)).thenReturn(List.of(aapl));
        ...
        verify(gateway, times(1)).getActiveEquities(0, 5000);
    }

    @Test
    void obtenerSimbolosPorMercados_paginatesThroughAllPages() {
        // First page: full page (5000 items)
        ...
        for (int i = 0; i < 500; i++) {
            page1.add(ActiveEquity.builder().symbol("SYM" + (5000 + i)).listedMarket("XNYS").build());
        }

        when(gateway.getActiveEquities(0, 5000)).thenReturn(page0);
        when(gateway.getActiveEquities(1, 5000)).thenReturn(page1);

        List<ActiveEquity> result = adapter.obtenerSimbolosPorMercados(List.of("NYSE"));

        assertEquals(5500, result.size());
        verify(gateway).getActiveEquities(0, 5000);
        verify(gateway).getActiveEquities(1, 5000);
    }
}
