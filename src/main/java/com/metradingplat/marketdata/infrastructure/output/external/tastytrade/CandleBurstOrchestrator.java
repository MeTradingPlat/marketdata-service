...
public class CandleBurstOrchestrator {
    ...
    public Map<String, List<Candle>> fetchBurst(List<String> symbols, EnumTimeframe timeframe, Instant fromTime,
        ...
        List<List<String>> shards = shard(symbols, config.getCandleBurst().getMaxConnections());
    ...
    private List<List<String>> shard(List<String> symbols, int maxConnections) {
        int wavesNeeded = (int) Math.ceil(symbols.size() / (double) CandleWaveFetcher.WAVE_MAX_SYMBOLS);
        int shardCount = Math.max(1, Math.min(maxConnections, wavesNeeded));
