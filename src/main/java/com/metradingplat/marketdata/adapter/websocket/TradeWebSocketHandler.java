 ...
 * 
 * Handles:
 * - Subscribe to trades for specific symbols
 * - Unsubscribe from trades
 * - Forward trade updates to subscribed clients
 */
@Slf4j
public class TradeWebSocketHandler extends BaseWebSocketHandler {
    ...
    protected void handleTextMessage(WebSocketSession session, TextMessage message) throws Exception {
        try {
            ...
            if ("subscribe".equals(action)) {
                ...
                log.info("Client {} subscribed to trades for symbol: {}", session.getId(), symbol);
                ...
                log.info("Client {} unsubscribed from trades for symbol: {}", session.getId(), symbol);
            }
        } catch (Exception e) {
            log.error("Error handling trade message", e);
            sendToSession(session, objectMapper.writeValueAsString(
                objectMapper.createObjectNode()
                    .put("type", "error")
    ...
    protected void onFirstSubscriberAdded(String symbol) {
        log.info("First subscriber added for trades: {}", symbol);
        // TODO: Subscribe to dxLink for this symbol
    }

    @Override
    protected void onLastSubscriberRemoved(String symbol) {
        log.info("Last subscriber removed for trades: {}", symbol);
